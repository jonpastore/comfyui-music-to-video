...
Deletes song+tier clip *files* only at clips.path (local os.remove).
artefacts.host is recorded on the dry-run listing so a remote twin can be
found later; this job does not SSH-delete another box. Never invents paths. Keeps anchors, refs, storyboards, the confirmed assembled
file, QC findings rows, and artefacts rows (status cleaned so findings still
join — T6-9).
