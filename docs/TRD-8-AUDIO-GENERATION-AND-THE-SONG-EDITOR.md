...
| topbar Media → New Song / New Image | **built** | `test_media_create.py` | Topbar **Media** is a submenu (`nav-drop`: New Song `/media?new=song`, New Image `/media?new=image`; click pins, 2s hold after leave, arrows+Enter, Escape does not reopen (`holdClosed`), `initNavDrop`). `GET /media` is the chooser (two link cards, no forms). Each pane is its own form (fetch / no full reload). ACE-Step creates a song; first take is `audio_original` via `as_new_song`. New Image runs `T2I_WIRED` Qwen only — no disabled Flux/Z-Image/Krea `<option>`s; hint names Flux-on-disk / Krea-not-installed. Style LoRA is a second loader only when named (`test_style_lora_is_absent_from_the_default_graph`); `GET /media/loras` refreshes the select. Civitai fetch reuses `/models/civitai` with base from the selected model. Mutation: empty tags still enqueue → red. Mutation: enqueue `flux2_t2i` → red. Mutation: flux `<option>` back on the picker → red. Mutation: both forms on `/media` → red |
