...
| topbar Media → New Song / New Image | **built** | `test_media_create.py` | Topbar **Media** is a submenu (`nav-drop`: New Song `/media?new=song`, New Image `/media?new=image`). `GET /media` is the chooser (two link cards, no forms). Each pane is its own form. ACE-Step creates a song; first take is `audio_original` via `as_new_song`. New Image is local t2i (album look retrieved into the prompt). Song page has no Generate take; repair (span / re-synth) stays on Edit audio. Nav `Media` after Library (`test_uiux_nav.py`). Mutation: empty tags still enqueue → red. Mutation: existing-song take overwrites `mp3_path` → red. Mutation: both forms on `/media` → red |
