...
| `GET /songs/{id}` not blocked on xAI `/models` | **built** | `test_song_page_async.py` | `grok.list_models` caches 300s (`MODELS_TTL`) and times out at 8s (`MODELS_TIMEOUT`). Song page already fail-softs to an empty dropdown. Mutation: timeout stays 120 or cache omitted → `/songs/32` 22s TTFB again (`test_list_models_is_cached_and_not_a_120s_wait`) |
