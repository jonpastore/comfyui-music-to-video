...
- **`take_voices`** — the junction carrying per-region voice parameters, so a
  ...
  to record *which* voice, which the junction is not. Generation now writes
  `params_json.voice_id` on the take (`None` when none was asked) and, when a
  voice is asked, also lands the whole-track assignment on `take_voices`.
...
| **`T8-9`** song span hint is service-owned (`T6-A4`) | **built** | this slice | `GET /songs/{id}` "Replace a span" hint interpolates `splice_eaten_secs` = `2 * mixer.SPLICE_XFADE` from the route; no `2 * 0.25` in `song.html`. Stub `SPLICE_XFADE=0.20` → page shows `0.40s`; hardcode `0.50s` from `2*0.25` is absent. Does not change `bridge_seconds` arithmetic. `studio/test_t8_9_splice_hint.py` |
...
| **`T8-11`** a take records which voice | **built** | `test_takes.py` | `h_audio` writes `params_json.voice_id` on every take — the id when a voice is asked, `None` when not. A take with a voice also gets a `take_voices` row. One generate with a voice and one without both record the distinction. |
...
| topbar Media → New Song / New Image | **built** | `test_media_create.py` | Topbar **Media** is a submenu (`nav-drop`: New Song `/media?new=song`, New Image `/media?new=image`; click pins, 2s hold after leave, arrows+Enter, Escape does not reopen (`holdClosed`), `initNavDrop`). `GET /media` is the chooser (two link cards, no forms). Each pane is its own form (fetch / no full reload). ACE-Step creates a song; first take is `audio_original` via `as_new_song`. New Image runs `T2I_WIRED` Qwen + Flux 2 Dev + Flux 2 Klein + Z-Image + Krea 2 Turbo OSS (`make_t2i.py` / `pipeline.gen_t2i`). Mage fruit models are not downloadable. Style LoRA is a second loader only when named (`test_style_lora_is_absent_from_the_default_graph`); `GET /media/loras` lists the selected family's pack (Anatomy / Popular optgroups, `seed/lora_pack.json`). Civitai search uses the real base (`Qwen`, `Flux.2 D`, `Flux.2 Klein 4B`, `ZImageTurbo`, `Krea 2`). `test_style_lora_select_filters_to_the_model_family`. Mutation: empty tags still enqueue → red. Mutation: enqueue `krea_t2i` → red. Mutation: both forms on `/media` → red |
| `GET /songs/{id}` not blocked on xAI `/models` or the fleet | **built** | `test_song_page_async.py` | `grok.list_models(wait=False)` returns cache/`[]` without blocking (`MODELS_TIMEOUT` 8s, `MODELS_TTL` 300s). Song GET does not call `swarm_backends` / `_object_info` (`test_song_page_does_not_probe_the_fleet`). Mutation: `wait=True` on GET or `catalog()` without `object_info={}` → `/songs/{id}` 22s TTFB again |
