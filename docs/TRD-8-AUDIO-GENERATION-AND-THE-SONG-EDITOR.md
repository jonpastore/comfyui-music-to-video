...
| topbar Media → New Song / New Image | **built** | `test_media_create.py` | `GET /media` is New Song (ACE-Step creates a song; first take is `audio_original` via `as_new_song`) and New Image (local t2i job, album look retrieved into the prompt). Song page has no Generate take; repair (span / re-synth) stays on Edit audio. Nav `Media` after Library (`test_uiux_nav.py`). Mutation: empty tags still enqueue → red. Mutation: existing-song take overwrites `mp3_path` → red |
