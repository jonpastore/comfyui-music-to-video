...
plate, picked from a thumbnail slider on the scene row — click opens a lightbox with search, a gallery grid, and a save icon; selecting a result fetch-pins it in place, no reload) → LTX 2.5 first (clips strip + shimmer cards on Render clip; a landed take shows the picture, a play badge, and a trash to delete that take) → optional decoded s2v hop on lip scenes →
