...
**Save Scene**, **Render clip**, and **Generate Images** persist every
scene field used to render (prompts with their own version save, camera,
duration, pose plate). Prompt versions do not increment when the text
is unchanged (`test_scene_prompt_placeholder_and_version_and_draft`).
Scene-form Save Scene is no longer required first
(`test_render_clip_writes_the_onscreen_motion_before_enqueue`,
