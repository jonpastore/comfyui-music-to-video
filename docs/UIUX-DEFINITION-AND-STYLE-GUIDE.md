...
prompt does not reach the tile as a clean candidate. `T4-13` is a
`channel_balance` FLAG on the sheet's pixels (olive/magenta vs grey wall),
not a `BACKDROP` string; the real GPU sheet is **NOT MEASURED**. In
