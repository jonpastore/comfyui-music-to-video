...
silent pass on solid red (whole-array std used to). `T3-4.2-black_frames`: partial black frames while mean luma PASSes FLAG `black_frames` with a re-render-seed remedy — not a silent pass on a dead-sampler span. `T3-4.2-sat`: a
