...
| **`T10-18`…`T10-26` the minor policy** | **partial** | this change | `T10-18` is built. `T10-18a`…`T10-26` (except the `T10-18c` floor constant) are still specification. `r` still refuses every field; lyrics-only mention at `r` is not yet the allowance |
