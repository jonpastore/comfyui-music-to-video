...
| `T10-4` toggle-all is the shown set | **built** | this change | `shown()` filters `offsetParent`; pick-all walks `shown()`. Twelve shown change, three hidden do not. Unfiltered `/` lists every row in the HTML. Accordion: only the open album's rows have `offsetParent` (`test_library_groups_by_album_and_collapses_upload`) |
...
| `T10-19` escalation re-screens at destination, names blocker | **built** | this change | `studio/test_t10_19_escalation_rescreen.py`: `guardrail.screen_escalation` names the field; g work with prompt-child cannot enqueue `r`/`xxx`; lyrics-only child cannot become `xxx` but can become `r`; clean work escalates; enabling nudity on `g` re-screens and refuses. `negative_prompt` is not a depiction field (PINNED-shaped do-not-draw lists). Mutation: drop the re-screen in `storyboard_service.enqueue` / `set_allow_nudity` and it goes red. Mutation: put `negative_prompt` back in `_WORK_SCENE_FIELDS` → R-board xxx enqueue arm red. Street Cats live Rear Entrance scene 1 is adult-only: `studio/test_t10_19_street_cats_explicit.py` screens checked-in `rear_entrance_explicit.json` scene 1 at `xxx` (baked PINNED age-lock → red) |
