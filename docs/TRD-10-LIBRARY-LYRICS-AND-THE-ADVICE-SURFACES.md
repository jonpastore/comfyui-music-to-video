...
| `T10-12` advice writes nothing directly | **built** | this change | `studio/test_t10_12_advice_writes.py`: `advice.retain` does not apply; `advice.accept` writes and records the model; `mixadvice.propose` leaves `set_items` unchanged; `POST /sets/{id}/suggest` retains a `proposal_id`; `POST /sets/{id}/proposals/{pid}/accept` writes the mix. `T10-13`…`T10-15` still not built |
