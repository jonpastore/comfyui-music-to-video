...
| `T10-16` image path refuses the child string audio accepts | **built** | this change | `studio/test_t10_16_image_guardrail.py`: same string `"nursery rhyme for children"` accepted on `POST /songs/{id}/audio/generate` and `make_audio.workflow`; refused on storyboard direction, scene `image_prompt` / `video_motion_prompt`, and `screen_prompt_field` |
