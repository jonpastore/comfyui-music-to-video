...
- `T6-19` **Operator-confirmed clip cleanup.** Confirm is a separate act from
  ...
  assembled file, findings rows. `cleanup_service`. **Song page UI:** after
...
- **No automatic GC.** Retention / garbage collection is out of scope here by
  name. Lifecycle writes still delete nothing (`T6-18`); predecessor
  reachability after a re-render is `T6-A5`. Operator Delete of one assembled
  output and `T6-19` confirmed clip cleanup are not automatic GC.
...
| `T6-A7` a measurement that cannot fail is not evidence | **built** | this change | `test_t6_a7_measurement_can_fail.py`: equal control/mutated refused; `jobs.canonical_path` two inputs → two outputs; T6-A4 stub keeps counts off list lengths so recomputing from `len` is the named mutation. Presence of the word "running" is not the criterion. |
| `T6-A8` mutate and read what changed | **built** | this change | `mutation_read.apply` / `read` refuse no-ops (`MutationNoOp`); report carries `n`, `old`, `new`, before/after. Truthy `str.replace` is not evidence; `replace(...) or source` short-circuit is the silent no-op the harness documents. `studio/test_t6_a8_mutation_read.py` |
| `T6-A9` a refusal or a presence is half a criterion | **built** | this change | `require_pair` refuses refusal-only and presence-only as complete proof; provisional must name the gap. Product exemplar: T6-2 refuse-early + successor-ready through `jobs._claim` (`test_t6_a9_positive_half.py`). |
| `T6-A10` assert through the shared entry point | **built** | this change | `test_t6_a10_shared_entry.py`: bypass via `_audio_chain(master=True)` stays green under broken `item_chains` wiring; product mixed set is levelled once through `item_chains`; one production `_audio_chain` call lives inside it; `models.canonical_host` is the host-identity collapse point. |
...
| `T6-A2` HTML and JSON report the same numbers | **built** | this change | Queue: `test_t6_a2_html_and_json_report_the_same_queue_numbers` — `/queue` HTML and `Accept: application/json` report the same 1 running / 2 waiting / 58s / 15s / 5s; same `queue_ctx()`. Review: `test_t6_a2_html_and_json_report_the_same_review_queue_numbers` — `/qc` HTML and `/api/qc/findings` report the same 2 open findings (4.8/30.0 s, 896/1024 px); same `qc_service.queue()`; pass rows stay out. **`T6-A2-set` built**: `test_t6_a2_html_and_json_report_the_same_set_numbers` — HTML `/sets/{id}` and JSON `/api/sets/{id}` report the same 3 items / 137s / item ids; same `set_detail()`. **`T6-A2-storyboard` built**: `test_t6_a2_html_and_json_report_the_same_storyboard_numbers` — HTML `/songs/{id}/storyboard/{tier}` and JSON `GET /api/songs/{id}/storyboard/{tier}` report the same `scene_time` 85 / `song_length` 120 / legal `clip_seconds(15)` / `scene_count` / `mismatch`; same `storyboard_service.payload()`. Stub arm forces `scene_count=99` (not list length) so a template that recomputes from `scene_rows` goes red. **`T6-A2-arc` built**: `test_t6_a2_html_and_json_report_the_same_arc_numbers` — HTML `/playlists/{id}/arc` and JSON `GET /api/playlists/{id}/arc` report the same `song_count` / `act_count` / `premise` / `has_proposal` from `arc_service.payload()`; stub arm forces `song_count=99` so a template `arc.songs \| length` recompute goes red. **`T6-A2-playlists` built**: `test_t6_a2_html_and_json_report_the_same_playlist_numbers` — HTML `/playlists` card (`#playlist-{id}` `data-song-count` / `data-total-secs`) and JSON `GET /api/playlists/{id}` report the same `song_count` / `total_secs` from `playlist_service.numbers()`; fixture 3 songs / 11.5+22.25+7.0=40.75s; stub arm forces `song_count=99` so a template `len` recompute goes red. `arc` still omitted when undefined (T2-37). **`T6-A2-library` built**: `test_t6_a2_html_and_json_report_the_same_library_numbers` — HTML `GET /` and `GET /songs` (`#library` `data-song-count`) and JSON `GET /api/songs` report the same `song_count` from `library_service.numbers()`; fixture 3 songs; stub arm forces `song_count=99` so a template `len` recompute goes red; `GET /songs` is 200 never 405. Distinctive numbers so two empty answers cannot pass. **`T6-A2-nav` built**: `test_uiux_nav.py` (`test_uiux_nav_html_and_json_share_one_list`) — HTML topbar (`base.html` iterates `nav_service.links()`) and JSON `GET /api/nav` report the same `{href,label}` list from `nav_service.LINKS`; distinctive probe monkeypatched into the list appears in both; hardcoding the old eight links in the template drops the probe → red. |
...
| `T6-A4` no template computes | **built** | this change | Queue: `test_t6_a4_queue_page_shows_stubbed_values_unmodified` — stub `queue_ctx` with elapsed `12.7s` and counts 3/7 (not the list lengths); `/queue` shows those strings unmodified. `_queue.html` interpolates `queue_n_running`, `queue_n_waiting`, `queue_rows`, `e.elapsed`. **`T6-A4-jobs` built**: `test_t6_a4_jobs_panel_shows_stubbed_elapsed_unmodified` — stub `jobs_ctx` with elapsed `12.7s` (not `int` of a seconds fixture); GET `/jobs` and `/jobs?partial=1` show `12.7s` in the cell and `data-elapsed`; a template that does `%.0f` of 12.7 → `13s` goes red. `_jobs_panel.html` interpolates `e.elapsed` only. **`T6-A4-storyboard` built**: `test_t6_a4_storyboard_page_shows_stubbed_fill_pct_unmodified` — stub `storyboard_service.payload` so `coverage.fill_pct=37.5` (not intent/rendered×100 of the fixture); HTML `/songs/{id}/storyboard/{tier}` meter width and `data-fill-pct` show `37.5` unmodified; recomputed ~70.83 does not appear as width. `#storyboard-meter` still carries T6-A2 numbers from the same stub. Template interpolates `coverage.fill_pct` only. **`T6-A4-splice-hint` built** (also T8-9): `test_t8_9_splice_hint.py` — stub `mixer.SPLICE_XFADE=0.20` so `2*xfade=0.40`; song generate card "Replace a span" hint shows `0.40s` unmodified; hardcode `0.50s` from template `2*0.25` goes red. `song.html` interpolates `splice_eaten_secs` only. |
...
| `T6-A3` service imports nothing from FastAPI; tests call it directly | **built** | this change | `sets_service.py`, `storyboard_service.py`, `arc_service.py`, `playlist_service.py`, `cleanup_service.py`, `media_service.py`, `library_service.py`, and `nav_service.py` have no `fastapi` import (AST): `test_t6_a3_sets_service_imports_nothing_from_fastapi`, `test_t6_a3_storyboard_service_imports_nothing_from_fastapi`, `test_t6_a3_arc_service_imports_nothing_from_fastapi`, `test_t6_a3_playlist_service_imports_nothing_from_fastapi`, `test_t6_a3_cleanup_service_imports_nothing_from_fastapi`, `test_t6_a3_media_service_imports_nothing_from_fastapi`, `test_t6_a3_library_service_imports_nothing_from_fastapi` (nav: no FastAPI in module; covered by `nav_service` itself and `test_uiux_nav.py`). `test_t6_a3_*` also creates a set, adds an item, reads payload, clamps `scene_seconds` 100→60, and flags a 20s/120s meter miss by calling the functions with no request. JSON handlers `api_sets_create` / `api_set_add_item` / `api_start_storyboard` / `api_storyboard_meter` no longer default or compute. |
| `T6-19` operator-confirmed clip cleanup | **built** | this change | `cleanup_service`: confirm ≠ assemble; dry-run lists `{path,host,remote,can_delete,reason}` and deletes none; local host → `os.remove`; remote without `SWARM_INPUT_DIRS` twin → skip not delete; remote with twin → ssh `rm -f --` with `shlex.quote` (OpenSSH shells remote argv); unconfirmed refuses. Mutation: unknown host as local delete → red. Unquoted ssh argv → red. `studio/test_t6_19_confirmed_cleanup.py`. **UI:** song page cleanup card after Confirm clean interpolates `plan_clip_cleanup` (path/host/remote/can_delete/reason, `n_can_delete`); form posts `dry_run=0`+`confirm=DELETE` to existing API; unconfirmed has no delete form. T6-A4 stub reason/`n_can_delete=17` → inventing from list length goes red. `studio/test_t6_19_cleanup_ui.py` |
| `T6-18` nothing is deleted by this document | **deferred** | this change | GC out of scope **named** (§7 **No automatic GC**). Lifecycle non-delete / predecessor reachability via `T6-A5` (`test_t6_a5_*`). Honesty check: `studio/test_t6_18_gc_deferred.py` asserts the TRD names GC deferred and a set re-render leaves the predecessor file. Not a pass for "storage works"; do not implement GC here. |
