...
| `T9-1`/`T9-2` retargeting | **built, check can fail** | `test_trd9_fleet.py` | Live 9.7 s render still stands. Red tests: `test_t9_1_retarget_rewrites_both_directions_and_as_written_is_absent` (peaches fp16 ↔ cerberus bf16; as-written name is absent from the pin). `test_t9_2_retarget_is_per_loader_not_a_global_rename` (VAE `ae` → `z_image_ae` on peaches; same string on UNETLoader is not rewritten). Mutation: global string-replace → T9-2 red |
...
| **`T9-13c` staging sequence** | **built, check can fail** | this tree | `models.STAGING_SEQUENCE` + `models.staging_allows(step, done=…)`: transfer → checksums → queues_idle → restart_swarm → render. Restart refused until transfer + checksums + queues_idle; render refused until restart. `studio/test_trd9_fleet.py::test_t9_13c_staging_sequence_is_transfer_checksums_idle_restart_render`. Mutation: allow restart before checksums or while queues busy → red. **Safety rests on the idle queue, not Swarm's cache** |
...
| `T9-10` a cache hit is not a refusal | **built, check can fail** | this tree | `pipeline.empty_render_kind` + `ab_paths_use_distinct_seeds`; `test_t9_10_cache_hit_is_not_a_refusal_and_ab_uses_different_seeds`: Swarm's "No images were generated…" and an empty comfy list on a byte-identical resubmit are `cache_hit`, not refusal; model/node misses stay `refusal`; different seeds are not a cache hit; same-seed A/B is refused |
| `T9-12` `/history` is not the authority | **built, check can fail** | this tree | `pipeline.swarm_job_ran` + `container_log_shows_execution` + `history_proves_swarm_idle`; `test_t9_12_history_is_not_authority_for_swarm_jobs`: empty `/history` before and after with a container log that shows execution is `ran`; empty `/history` alone does not prove idle; `/history` alone cannot prove a Swarm job ran |
| `T9-9` empty backend refused; stocked registers and renders | **built, check can fail** | this tree | `studio/test_trd9_fleet.py::test_t9_9_empty_backend_refused_stocked_registers_and_renders`: reachable + no catalogued model → `empty`/`hazard="empty"` and `accept_backend` raises; stocked accept returns the row and `where()` still offers it; unreachable is not this hazard |
| `T9-15` pre-render VRAM on the submit result | **built, check can fail** | `test_t5_5_refine_peak_vram.py` | `test_t5_5_submit_records_pre_render_vram`: `_submit_and_collect` writes `LAST_RENDER_VRAM` with `pre.used_gb` and a measured `peak`. Mutation: drop `LAST_RENDER_VRAM` → red. `sample_vram` stubbed; does not talk to a live box |
