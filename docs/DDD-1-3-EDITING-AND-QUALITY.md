...
| `studio/qc.py` | 642 | TRD-3 tier 1 in full; **T3-11** `check_set` / `run(kind="set")` duration vs `mixer.set_duration()` within `mixer.SET_DURATION_TOLERANCE` on the artefact (`test_t3_11_set_duration.py`); T3-13 `score_zimage_sweep`; T3-15 histogram `identity_embed`; T3-16 `identity_verdict`; T3-17 `score_identity_artefact` (per artefact vs chosen anchor); T3-26 `measure_refiner_help` (fail-closed labelled set, not opportunistic); T3-28 `check_identity_wrong` / `identity_wrong_remedy`; T3-27 `CHECK_REMEDY_CLASS` / `actuator_for` | built |
