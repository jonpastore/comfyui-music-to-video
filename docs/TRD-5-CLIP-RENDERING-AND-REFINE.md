...
| `T5-14` T5-A refine on the LTX take, not on s2v | **built** | `test_t5_14_refine_on_ltx_take.py` | `--refine` + `needs_lip_sync` attaches `SplitSigmasDenoise` / `_refined` SaveVideo only on the LTX graph. Hop successors force `refine=False`: no WAN i2v-low, no `_refined` hop SaveVideo. Hop `control_video` is the refined LTX prefix. Labels no longer say unproven-on-s2v / i2v-low refine (`h_clips`, `song.html`). Mutation: refine on hop → red. Mutation: hop `control_video` points at the non-`_refined` LTX prefix while LTX SaveVideo is `*_refined` → red |
...
| `T5-5` peak VRAM of the shipped variant | **harness only; peak NOT MEASURED** | `test_t5_5_refine_peak_vram.py` | Same as 2026-08-13 row. DDD honesty: T5-5 is not the base 23.4 peak; `refine_peak` stays `origin=not_measured`. Docs claiming T5-5 is that measurement go red |
...
| `T5-5` peak VRAM of the shipped variant | **harness only; peak NOT MEASURED** | `test_t5_5_refine_peak_vram.py` | `pipeline.sample_vram` / `peak_from_samples` / `t5_5_claim`; `models.refine_peak` sits beside the 23.4/23.9 base figure with `origin=not_measured`. Empty samples raise. `origin=measured` with `same_as_base` or no `n_samples` is a quoted 23.4 and goes red. Docs must not claim T5-5 is the base 23.4 measurement (`test_t5_5_docs_do_not_claim_base_peak_is_t5_5`). Submit records a pre-render reading on `LAST_RENDER_VRAM` (T9-15). A's peak has not been read off the box — live jobs held the card. Mutation: quote 23.4 as measured → red. HOLD: do not flip MEASURED without a GPU sample pair |
