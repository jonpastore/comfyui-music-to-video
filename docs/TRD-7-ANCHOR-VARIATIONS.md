...
| `T7-23` use-as-ref only keepers `usable≠skip` | **built** | `test_t7_23_usable_skip.py` | `classification.refuse_skip(album, path=/id)` refuses `usable=skip`. Called from `_use_anchor_as_ref`, `_collect_anchor_ref_paths`, `scene_pose_map._upsert_draft` / `accepted_bases`. Mutation: skip chosen as use-as-ref / map keeper / image1 → red. Mutation: `usable≠skip` still enters those slots → green |
...
| `T7-20` named uploaded poses | **built** | this change | An operator names a base image, optionally assigns a tier, and either generates a sheet for that pose (identity photos + that plate) or assigns the upload itself as the chosen sheet. Upload-pose / assign write `scope_kind=shared` (`T4-25`): one file, any album. An assigned upload drops out of Base images (`anchor_refs` skips `source=upload` chosen rows). Deleting that sheet also deletes the source `assets` row (same path) — leaving the row was Street Cats #141, an empty crouching card after a wrong-tier upload-pose. `anchor_refs` also drops a row whose file is already gone. A multi-body plate (stamped `actors`, cowgirl/spit-roast family, or another cast name on the label) nests on the **Actors** tab (`test_nest_puts_split_roast_on_the_actors_tab` / `test_upload_pose_stamps_every_actor`). Generate uses `actor_id` / `actor_all` (no character dropdown). Base cards tick `actor_name`; Assign as sheet copies them (`test_assign_as_sheet_stamps_actors_from_the_card`). Gallery tiles show those names via `actorlist(album)` (`Meow P · Panther`). Lightbox `#actor-tag` writes the same stamp (`POST /anchors/{id}/actors`). Upload cap is 24. Custom `pose_<id>` views omit the standing backdrop clause. Mutation: 95/20/40 score stores 20; assign creates a chosen `anchors` row from the file; `test_named_pose_meta_and_assign` asserts the asset is gone from `anchor_refs`; `test_deleting_uploaded_pose_sheet_drops_the_base_image_row` / `test_anchor_refs_drops_a_row_whose_file_is_already_gone`; `test_shared_anchors.py` asserts no per-album copy |
