...
| `T7-7` identity held across views | **NOT MEASURED** (image); **hook built** | `b081030` | GPU image differential has never been run. Offline: `check_identity_look` FLAGs a nude compose that asserts a human body — including the live-studio "Human woman's body" clause on `body` — through `check_image` / `qc.run` / `run_artefact`. A clean album field does not hide a dirty compose. |
