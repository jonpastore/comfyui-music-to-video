...
2. **TRD-8's take model.** New generations land as `takes` rows (`T8-1`); pick
   ...
   **`T8-16` is built** — the media menu is one bag
   ...
   (`/media?new=image`, `T2I_WIRED` Qwen / Flux 2 / Z-Image / Krea 2 Turbo
   OSS + Style LoRA + Civitai fetch with matching base; no fake disabled
   rows; Mage fruit is not local). `/media` is the
