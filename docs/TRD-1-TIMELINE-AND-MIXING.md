...
repeats that is worse than none. Production is `1c15c15` (2026-08-15); `origin/main` is current.
...
| `T1-2` delete item deletes its automation | **built** | `test_trd6_queue.py` | `test_t6_10_delete_set_item_removes_its_automation`: intended delete removes the item's automation rows; orphans do not remain. Shared with T6-10 |
...
| `T1-5` off-grid join: video nearest frame, audio exact second | **built** | this slice | `studio/test_t1_5_off_grid_join.py`: 2.02 s item / 0.5 s fade at 30 fps → exact offset 1.52 s; `_build_render_set_filter` xfade offset is nearest frame 1.533 s, not exact 1.520 and not truncated 1.500; `acrossfade` stays on the stored second. `rounding_report` / `GET /api/sets/{id}` `rounding.joins[].delta` non-zero. |
...
| peaks on the set timeline | **built** | this slice | `.tl-block` draws `mixer.peaks` (`data-peaks` / `data-reason`), not `waveform_png` as `background-image`. Empty envelope surfaces `reason`. HTML `/sets/{id}` and JSON `/api/sets/{id}` report the same pairs (`T6-A2`). Default GET does not remux. `studio/test_t1_peaks_meter.py` |
| live loudness meter | **built** | this slice | Last-render I/TP shown without remux. `GET /api/sets/{id}/loudness` remuxes and names I/TP via `mixer.export_loudness`. In-tolerance not flagged; hot mix flagged through `live_loudness`. Empty set is no meter. `studio/test_t1_peaks_meter.py` |
...
| `T1-20c` easy master is that same chain | **built** | `test_t1_18_audience.py` | easy reuses `_master_lines` / `one_button_master`; not a second implementation. T1-19 is the named chain |
...
| `T1-21`…`T1-23` `duck`/`layer` | **built (ledger was stale)** | mixer joins | `_duck_join` / `_layer_join` render. `T1-22` is the layer join (both items' frames present in the overlap). `T1-23` "refused" is no longer the tree |
| `T1-26` re-render writes a sibling | **built** | T6-A5 | `h_render_set` lands a new file; predecessor and successor both listed (`test_t6_a5_*`). Does not overwrite the previous render |
