...
| **tier 3, §6 entire** | **built** | `test_qc_approve.py` + child rows | `approve()` enqueues one repair and dest ≠ source (`T3-6`/`T3-18`). `T3-19`…`T3-27` and `T3-33` each have a red test. HTML `POST /qc/findings/{id}/approve` and JSON `POST /api/qc/findings/{id}/approve` both call `qc_service.approve`. The API's leftover `NotImplementedError`→501 branch is dead: `approve()` no longer raises that |
