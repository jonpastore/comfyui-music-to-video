...
| `T2-52` Accept-gated pose→scene map | **built** | `test_t2_52_map_accept.py` / `test_freeze_auto_binds_is_gone` / `test_pose_plan.py` | Accept persists `status=accepted`. Reject of a later draft restores the previous accepted keeper. `POST /songs/{id}/refs` and `POST /songs/{id}/reroll` 400 on empty map, draft, or rejected and write no job; accepted enqueue + `h_refs` writes a still. `pose_plan.freeze_auto_binds` is deleted (not a fallback). `scene_bases` is saved `pose_sheet_id` only; `h_refs`/`h_reroll` take job `pose_bases` or `{}` (no auto fallback). Mutation: restore `freeze_auto_binds` → red. Mutation: start_refs / start_reroll from empty map → red. Mutation: include `source=auto` in `scene_bases` → red. Mutation: reject overwrites previous accepted keeper → red |
...
| leftover auto `scene_bases` / start_reroll Accept | **built** | `test_pose_plan.py` | `scene_bases` returns only saved `pose_sheet_id` binds (unbound matching sheet → `{}`). `start_reroll` Accept-gates like `start_refs` and enqueues `pose_bases: {}`. `h_refs`/`h_reroll` do not fall back to auto `scene_bases` (`or {}` / missing-key). Mutation: restore auto→render in `scene_bases` → red. Mutation: empty `pose_bases` on reroll fills from auto → red. Mutation: start_reroll with empty map enqueues → red |
