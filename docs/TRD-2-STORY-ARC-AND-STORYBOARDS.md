...
| `T2-51` classify cannot write the map | **partial** | `test_t2_51_classify_cannot_write_map.py` | Classify + gap write zero `scene_pose_map` rows (same mutation as `T4-23` **built**). Draft keeper→scene (`status=draft`) is **not** this test — that is `T2-52`. `pose_plan` still auto-binds without Accept |
