...
Refreshed 2026-08-15 against `7de0aea`. The five grouped “not built” wand/meter/cast rows above were stale: each named criterion now has its own **built** row. Remaining product work on this TRD is **T2-28** (refs enqueue must refuse an unanchored lead, not only paint a banner) and the **reference-image generate path** (`h_refs` / `POST /songs/{id}/refs`) using the chosen anchor as identity, not a standing plate.
