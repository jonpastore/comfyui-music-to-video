...
| `T2-56` per-scene accepted keeper as image1 | **built** | `test_t2_56_per_scene_keeper.py` | Two accepted scenes with two keepers produce two different image1 paths. `build_refs --anchors` + `pipeline.gen_refs(anchors=)`. `start_refs`/`h_refs` pass `accepted_bases` as per-scene identity, not `pose_bases`/image2. Album-front-for-all and draft/rejected generate stay red. Location plates (`T2-53`) and extra-view slots left. Mutation: every scene's image1 is the album front → red. Mutation: draft/rejected still generates → red |
