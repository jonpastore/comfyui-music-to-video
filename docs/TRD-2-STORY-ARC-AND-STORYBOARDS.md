...
| **`T2-10` scene over ceiling is a chain** | **built (graph)** | `test_t2_10_clip_chain.py` | `chain_clip_count(30)` is 2 at the 15 s LTX cost ceiling. Successor graph has `LTXVAddGuide` at frame 0. Last frame of N vs first frame of N+1 is extracted and compared. `n_clips_for` is unchanged. Queue readiness (`T2-11`) is not this slice |
...
| §4.2–4.3 remaining wands (`T2-17`…`T2-19`), §5.3 casting | **not built** | — | |
...
| §4 remaining wands (`T2-14`, `T2-16`…`T2-19`), `T2-24`/`T2-25` meter remainder, §5.3 casting | **not built** | — | |
| **`T2-26` storyboard API returns album anchors per character** | **built** | `anchors_by_character` | `GET /api/songs/{id}/storyboard/{tier}` includes `anchors`: one group per character (`character`, `character_id`, `images` with `path`/`url`/`view`). Protagonist first (`character_id` NULL). Chosen sheets only; unchosen, other album, other tier stay out. A character with no chosen sheet is omitted. Mutation: omit the key → red. Mutation: flat list of images with no character grouping → red. Mutation: protagonist only when a cast sheet is chosen → red. `T2-27` (per-scene refs) is not this |
...
| §4 remaining wands (`T2-14`…`T2-19`), `T2-25` meter remainder, §5.3 casting | **not built** | — | |
...
| §4 remaining wands (`T2-14`…`T2-19`), `T2-25` meter remainder, `T2-30` unanchored-lead warning | **not built** | — | |
...
| **`T2-11` chained clip not ready until predecessor landed** | **built** | `test_t2_11_clip_chain_depends.py` | `build_song.clip_chain_plan` marks same-scene successors `depends_on` predecessor clip_idx. `app.enqueue_clips` (from `start_clips`) enqueues one job per chain clip with `jobs.enqueue(..., depends_on=pred_jid)`; under-ceiling songs stay one batch job. `_claim` (T6-2) skips the successor until the predecessor is `done`. Mutation: one batch job with no `_depends_on` → red. Mutation: always fan out per-clip with no chain → no-chain arm red |
| **`T2-17` generation prompt API** | **built** | this change | `GET /api/songs/{id}/storyboard/{tier}` returns `prompt` from `storyboard_generation_payload`, defaulted from the tier (`g` ≠ `r`). A stored prompt wins after generate. `POST` accepts an edited `prompt` as the job's `direction`. Mutation: hardcode one prompt for every tier → red. `studio/test_t2_17_storyboard_prompt.py` |
