...
| **pose plate per scene as image2** | **partial** (Accept-gated map built; leftover auto-bind if no map) | `test_t2_52_map_accept.py` | `scene_pose_map` is the map (`T2-51`/`T2-52` **built**). `start_refs` uses accepted rows and refuses draft/rejected. Unmapped songs still `pose_plan.freeze_auto_binds` (leftover). Product leftover is `T2-56`: that scene's accepted keeper is image1 |
