...
| `T2-52` Accept-gated pose→scene map | **built** | `test_t2_52_map_accept.py` / `test_freeze_auto_binds_is_gone` | Accept persists `status=accepted`. Reject of a later draft restores the previous accepted keeper. `POST /songs/{id}/refs` 400s on empty map, draft, or rejected and writes no still; accepted enqueue + `h_refs` writes a still. `pose_plan.freeze_auto_binds` is deleted (not a fallback). Mutation: restore `freeze_auto_binds` → red. Mutation: start_refs from draft → red. Mutation: start_refs with empty map → red. Mutation: reject overwrites previous accepted keeper → red |
...
| **pose plate per scene as image2** | **built** | `test_t2_52_map_accept.py` / `test_pose_plan.py` | `scene_pose_map` is the map (`T2-51`/`T2-52` **built**). `start_refs` requires draft+Accept: empty / draft / rejected → 400; accepted rows become `anchors` (image1), not `pose_bases`. `pose_plan.freeze_auto_binds` deleted (`test_freeze_auto_binds_is_gone`; `test_start_refs_freezes_pose_bases` expects 400; empty-map arm on `test_t2_52_map_accept`). Scene-row / album-coverage pool is **scene actors** (kneel is partnered): a Panther-lead cowgirl / kneeling-look-back / supine scene binds a Meow P or ensemble keeper (`test_panther_lead_partnered_binds_meowp_keepers`). Scene row: pose is a textarea; current plate + Save plate/Pinned sit under it; the plate is a taller thumbnail slider (`pose-picks`). Click opens `#pose-gallery` (search + Gallery grid + save icon). Selecting a result `fetch`es the bind (hidden `sheet_id`) and paints the current thumb — no form submit / 303. Not `#ref-preview`. Not a `<select>`. A landed clip tile is `<video src>` + `preload=metadata` + `poster` from the approved still and a play badge (`test_song_page_async.py`); not `lazy-src`. Mutation: empty map still enqueues refs → red. Mutation: pool = `scene_leads` only → red. Mutation: restore `<select name=sheet_id>` → red. Mutation: drop `#pose-gallery-q` / `#pose-gallery-grid` / `#pose-gallery-use` → red. Mutation: `applyPose` uses `requestSubmit` / `form.submit` → red (`test_pose_plan.py`). Location plates (`T2-53` **built**) |
