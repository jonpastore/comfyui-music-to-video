...
| **`T2-28` unanchored lead flagged before refs render** | **partial** | `unanchored_leads` | UI/API list the lead (`T2-30`). `POST /songs/{id}/refs` still only requires the album protagonist chosen sheet, not every named lead. A dedicated refuse-before-enqueue test is the remaining half |
...
Refreshed 2026-08-15 against refs-identity + cast-slots. The five grouped “not built” wand/meter/cast rows above were stale: each named criterion now has its own **built** row. The **reference-image generate path** (`h_refs` / `pipeline.gen_refs` / `build_refs.workflow`) is **built**: image1 is the chosen sheet, not a standing 4748 plate (`test_t2_refs_identity.py`). Cast slots are **built**: only named leads with chosen sheets take image2/3. Remaining product work on this TRD is **T2-28** (refs enqueue must refuse an unanchored lead, not only paint a banner).
