...
- `T2-52` **Accept is required per scene, same shape as `T2-15`.**
  ...
  still (`require_accepted` / `start_refs` 400). No
  `pose_plan.freeze_auto_binds` fallback.
  *Mutation: generate refs from a draft row → red. Mutation: generate
  refs with an empty map → red. Mutation: reject overwrites the
  previous accepted keeper → red.*
...
| `T2-52` Accept-gated pose→scene map | **built** | `test_t2_52_map_accept.py` | Accept persists `status=accepted`. Reject of a later draft restores the previous accepted keeper. `POST /songs/{id}/refs` 400s on empty map, draft, or rejected and writes no still; accepted enqueue + `h_refs` writes a still. No `freeze_auto_binds` fallback. Mutation: start_refs from draft → red. Mutation: start_refs with empty map → red. Mutation: reject overwrites previous accepted keeper → red |
