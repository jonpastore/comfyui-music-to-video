...
| **pose plate per scene as image2** | **partial** (Accept-gated map built; leftover auto-bind if no map) | `test_t2_52_map_accept.py` / `test_pose_plan.py` | `scene_pose_map` is the map (`T2-51`/`T2-52` **built**). `start_refs` uses accepted rows as `anchors` (image1), not `pose_bases`. Unmapped songs still `pose_plan.freeze_auto_binds` (leftover image2). Leftover pool is **scene actors** (kneel is partnered): a Panther-lead cowgirl / kneeling-look-back / supine scene binds a Meow P or ensemble keeper (`test_panther_lead_partnered_binds_meowp_keepers`). `album_coverage` keeper list is the same pool. Scene row: pose is a textarea; current plate + Save plate/Pinned sit under it; the plate is a taller thumbnail slider (`pose-picks`). Click opens `#pose-gallery` (search + Gallery grid + save icon). Selecting a result `fetch`es the bind (hidden `sheet_id`) and paints the current thumb — no form submit / 303. Not `#ref-preview`. Not a `<select>`. A landed clip tile is `<video src>` + `preload=metadata` + `poster` from the approved still and a play badge (`test_song_page_async.py`); not `lazy-src`. Mutation: pool = `scene_leads` only → red. Mutation: restore `<select name=sheet_id>` → red. Mutation: drop `#pose-gallery-q` / `#pose-gallery-grid` / `#pose-gallery-use` → red. Mutation: `applyPose` uses `requestSubmit` / `form.submit` → red (`test_pose_plan.py`). Location plates (`T2-53` **built**) |
