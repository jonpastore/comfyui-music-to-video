...
- `T2-13e` A plan whose clip durations miss the track length by more than one
  ...
  refuse; bare full-song render still refuses (seam with `T2-25`). The live
  Coverage meter hint tells the operator to regenerate or edit
  `duration_guidance` to fill the track, or use scene-scoped Render clip —
  it does not claim pacing "matches the track" from `coverage.ok`
  (intent≈rendered only) or that scenes are stretched/compressed when
  full-song refuses.
...
- `T2-52` **Accept is required per scene, same shape as `T2-15`.**
  ...
  accepted bindings; an empty map, draft, or rejected scene writes no
  still (`require_accepted` / `start_refs` 400). `pose_plan.freeze_auto_binds`
  is deleted — not a fallback.
  *Mutation: restore `freeze_auto_binds` → red. Mutation: generate refs
  from a draft row → red. Mutation: generate refs with an empty map →
  red. Mutation: reject overwrites the previous accepted keeper → red.*
  (`test_t2_52_map_accept.py`, `test_freeze_auto_binds_is_gone`)
...
| `T2-52` Accept-gated pose→scene map | **built** | `test_t2_52_map_accept.py` / `test_freeze_auto_binds_is_gone` / `test_pose_plan.py` | Accept persists `status=accepted`. Reject of a later draft restores the previous accepted keeper. `POST /songs/{id}/refs` (`start_refs`) 400 on empty map, draft, or rejected and writes no job; accepted enqueue + `h_refs` writes a still. Accept is **start_refs only** — scene-row `start_reroll` uses a pinned `pose_sheet_id` (`scene_bases`) and does not wait on the map (`test_start_reroll_pinned_plate_skips_empty_map`). `pose_plan.freeze_auto_binds` is deleted (not a fallback). `scene_bases` is saved `pose_sheet_id` only; `h_refs`/`h_reroll` take job `pose_bases` or `{}` (no auto fallback). Mutation: restore `freeze_auto_binds` → red. Mutation: start_refs from empty map → red. Mutation: include `source=auto` in `scene_bases` → red. Mutation: reject overwrites previous accepted keeper → red |
...
| `T2-54` ceiling + ticked-lower backfill | **built** | `test_t2_54_ceiling_backfill.py` | `storyboard_backfill.backfill(song_id, run_tiers)` takes the ceiling board and writes only ticked tiers ≤ ceiling, each with `compose_guardrail(tier)` + allowed wardrobe. r+pg13 writes both (pg13 clothed). r-only writes no pg13. g writes a clothed g board; nude view or r/xxx board is red. No route this slice. Mutation: r-only writes pg13 → red. Mutation: g writes nude view or r/xxx → red |
| `T2-55` `needs_lip_sync` = LTX first then D7 | **built** | `test_t2_55_needs_lip_sync.py` | Field beside camera in `EDITABLE_SCENE_FIELDS` / `SHORT_SCENE_FIELDS` / `_scene_json` / `_scene_row`. true → LTX then s2v hop (`clip_chain_plan` / `main`); false/absent → LTX only. Hop 0 is LTX (`T5-11` **built**). Hop graph is `T5-12` **built**. Mutation: true emits s2v-only → red. Mutation: omit from `_scene_json` → GET red. Mutation: false → hop → red |
...
| `T2-32` D10 wording (text + her image1) | **built** | `test_t2_31_empty_reference.py` | `_assert_t2_32_message` requires the text lock (species/body) plus her photographs as image1. Old "identity comes from the text, not the reference image" fails. Empty lock still refused (`T2-31`). Mutation: restore old wording → red |
| `T2-42` / `T2-43` `video_model` field | **built** | `test_t2_42_scene_video_model.py` | Field posts and lives beside camera. Hop 0 is LTX (`T5-11` **built**); lip-sync is `T2-55` **built** |
| leftover auto-bind actor pool (kneel partnered) | **built** | `test_pose_plan.py` | `plan` / `album_coverage` pool is `scene_actors` (kneel ∈ `_PARTNERED`). Panther-lead cowgirl / kneeling-look-back / supine match Meow P / ensemble keepers; no GPU. Mutation: pool = `scene_leads` only → red. Mutation: kneel not partnered → kneeling-look-back unbound |
| last-selected prompt version restores on refresh | **built** | `test_song_page_async.py` | Scene `field_current` + `apply_field_version`. Lyrics/style/direction/lock/audio/album look/artwork instruction use `prompt_current`. Mutation: apply does not write the live field → GET still shows the old wording |
| still stale chip explains itself | **built** | `test_app.py` | Scene `edited` after every candidate `created` paints a clickable **stale** chip (`data-label="Stale"`) that says to reroll from current scene text. Mutation: drop `data-label="Stale"` / the Save Scene sentence → red |
...
| **`T2-13e` plan that misses the track by more than one clip is refused before render** | **built** | `test_t2_13e_plan_miss.py` / `test_t2_25_scene_time_enqueue.py` / `test_t2_13e_meter_copy.py` | `clip_plan` with a known track raises when `|sum(clip_seconds) - track|` exceeds one clip; `main()` writes no clip graphs. A miss of exactly one clip, and nclips-only display, still allocate. `--only` and studio `POST /clips` with `scene=` / `clip_idx=` (Render clip) skip the full-track / scene-time refuse (`test_t2_13e_only_skips_full_track_refuse`, `test_t2_25_scene_scoped_skips_mismatch_refuse`); bare full-song POST still 400s. `assemble_song` keeps `-t audio_dur` and no longer says clips are quantised so the video always overruns. **Meter honesty:** `storyboard.html` Coverage hint on a mismatch (77s board / 237s track) says regenerate or edit `duration_guidance` to fill the track, or scene-scoped Render clip; bare full-song refuses. It does not say "Pacing matches the track" (`coverage.ok` is intent≈rendered only) or claim scenes are stretched/compressed when full-song refuses (`test_t2_13e_meter_copy.py`). Mutation: allocate and return → red. Mutation: `--only` / scene-scoped still refuse a short board → red. Mutation: restore pacing/stretch copy → red |
...
| **`T2-31` / `T2-32` empty `character_reference` refused at save** | **built** | `test_t2_31_empty_reference.py` | Empty lock refused. Message names both D10 halves (text lock + her photographs as image1). Mutation: restore "the text, not the reference image" → T2-32 red |
...
| **refs-identity chosen sheet as image1** | **built** (`T2-56`) | `test_t2_56_per_scene_keeper.py` | Accepted map rows: that scene's keeper is image1 (`--anchors`). Standing 4748 plate is refused (keep, `test_t2_refs_identity.py`). Empty map refuses refs (`T2-52`). Location plates (`T2-53` **built**). Extra-view slots left. Mutation: every scene's image1 is the album front when the map is accepted → red |
| **pose plate per scene as image2** | **built** | `test_t2_52_map_accept.py` / `test_pose_plan.py` | `scene_pose_map` is the map (`T2-51`/`T2-52` **built**). `start_refs` requires draft+Accept: empty / draft / rejected → 400; accepted rows become `anchors` (image1), not `pose_bases`. Scene-row **Reroll** does not wait on that map — it needs a **pinned** plate on the requested scene (`scene_bases`) and enqueues those paths as `pose_bases` (`test_start_reroll_pinned_plate_skips_empty_map`). No pin → 400 "pin a pose plate" (`test_start_reroll_refuses_unpinned_scene`). `pose_plan.scene_bases` is saved `pose_sheet_id` only (not `plan()` `source=auto`). `h_refs`/`h_reroll` use job `pose_bases` or `{}` — no missing-key / `or scene_bases` auto fallback (`test_scene_bases_unbound_matching_sheet_is_empty`, `test_h_reroll_empty_pose_bases_stays_empty`). `pose_plan.freeze_auto_binds` deleted (`test_freeze_auto_binds_is_gone`; `test_start_refs_freezes_pose_bases` expects 400; empty-map arm on `test_t2_52_map_accept`). Scene-row / album-coverage pool is **scene actors** (kneel is partnered): a Panther-lead cowgirl / kneeling-look-back / supine scene binds a Meow P or ensemble keeper (`test_panther_lead_partnered_binds_meowp_keepers`). Scene row: pose is a textarea; current plate + Save plate/Pinned sit under it; the plate is a taller thumbnail slider (`pose-picks`). Click opens `#pose-gallery` (search + Gallery grid + save icon). Selecting a result `fetch`es the bind (hidden `sheet_id`) and paints the current thumb — no form submit / 303. Not `#ref-preview`. Not a `<select>`. A landed clip tile is `<video src>` + `preload=metadata` + `poster` from the approved still and a play badge (`test_song_page_async.py`); not `lazy-src`. Scene prompt boxes persist the last selected `field_versions` row as `field_current` (`test_scene_prompt_placeholder_and_version_and_draft`). Song lyrics/style/direction/lock and album look boxes remember the last selected `prompt_versions` id in `prompt_current`. Mutation: empty map still enqueues refs/reroll → red. Mutation: include auto in `scene_bases` → red. Mutation: empty reroll `pose_bases` fills from auto → red. Mutation: pool = `scene_leads` only → red. Mutation: restore `<select name=sheet_id>` → red. Mutation: drop `#pose-gallery-q` / `#pose-gallery-grid` / `#pose-gallery-use` → red. Mutation: `applyPose` uses `requestSubmit` / `form.submit` → red (`test_pose_plan.py`). Location plates (`T2-53` **built**) |
...
| **`T2-42` scene may carry `video_model`** | **built** (field) | `test_t2_42_scene_video_model.py` | scene JSON carries `video_model` beside `camera`; GET returns it; unmarked stays empty. Hop 0 is `ltx25` (`T5-11`); the field does not skip LTX. Mutation: omit from `_scene_json` → GET arm red. Mutation: invent a default onto an unmarked scene → absent arm red |
...
| **`T2-55` `needs_lip_sync` beside camera** | **built** | `test_t2_55_needs_lip_sync.py` | Scene JSON carries bool `needs_lip_sync` beside `video_model`/`camera`; GET always returns it (absent → false); editable via `EDITABLE_SCENE_FIELDS` / scene-row checkbox. true → LTX then s2v hop; false/absent → LTX only. Does not skip LTX. Mutation: true emits s2v-only → red. Mutation: omit from `_scene_json` → GET red. Mutation: false emits hop → red |
...
| `T2-48` per-scene model ceilings compose | **built** | `test_t2_48_ceilings_compose.py` | Hop 0 splits a 30 s scene on the LTX ceiling (15 s + 15 s) even when marked `s2v`; a 30 s `needs_lip_sync` scene is LTX 15+15 plus per-part s2v windows (`clip_chain_plan` / `split_to_ceiling(s2v)`). Tiles (`T2-8b`); `_compose` stamps `clips`; `validate` refuses a gap; `main()` emits LTX graphs. T5-12 hop graph **built**. Mutation: treat `video_model=s2v` as hop 0 → 7 × CHUNK and this fails. Mutation: 30 s lip scene omits per-part s2v windows → red |
...
Refreshed 2026-08-18 against Accept-gated empty-map refuse + T2-28-html + refs-identity + cast-slots + refs-score + refs-length per-clip expect + closed auto-`scene_bases` leftover (pinned `pose_sheet_id`; Accept is `start_refs` only). The five grouped “not built” wand/meter/cast rows above were stale: each named criterion now has its own **built** row. **T2-28** is built: `POST /songs/{id}/refs` refuses when a named lead has no chosen sheet; the storyboard plan-panel marks Generate refs (`button.blocked`, names the unanchored lead). The **reference-image generate path** (`h_refs` / `pipeline.gen_refs` / `build_refs.workflow`) is **built**: image1 is the accepted per-scene keeper (`T2-56`), not a standing 4748 plate (`test_t2_refs_identity.py`). **Pose map / plates** are **built**: draft+Accept required before refs (`T2-52`, empty map 400s on `start_refs`); scene-row reroll uses a pinned `pose_sheet_id` and does not wait on the map — empty map + pin enqueues (`test_start_reroll_pinned_plate_skips_empty_map`); accepted keepers are `anchors` image1, not `pose_bases`; `freeze_auto_binds` deleted; `scene_bases` is saved binds only; `h_refs`/`h_reroll` do not auto-fill plates (`test_pose_plan.py`). Scene-row pose-sheet bind and album-coverage actor pool remain (`pose_plan`); cast leads take leftover slots. Landed-ref **scoring** vs the chosen anchor (`ref_score_bases`) is built (`test_h_refs_scores_vs_chosen_anchor`). **refs-length per-clip**: each ref graph's expect is `clip_seconds` / `legal_frames`, not CHUNK (`test_t2_refs_clip_seconds.py`).
